...
import com.dagitab.pos.connection.DataUtil;
import com.dagitab.pos.connection.LogHandler;
import com.dagitab.pos.forms.pullouts.PullOutReq;
...
public class NewPullOutReq extends javax.swing.JDialog {
	private JLabel newPullOutRewLabel;
...
//							dialog.setVisible(true);
				...
				{
					newPullOutRewLabel = new JLabel();
					getContentPane().add(newPullOutRewLabel, new AnchorConstraint(28, 312, 133, 11, AnchorConstraint.ANCHOR_REL, AnchorConstraint.ANCHOR_REL, AnchorConstraint.ANCHOR_REL, AnchorConstraint.ANCHOR_REL));
					newPullOutRewLabel.setText("New Pull Out Request");
					newPullOutRewLabel.setPreferredSize(new java.awt.Dimension(189, 28));
					newPullOutRewLabel.setFont(new java.awt.Font("Tahoma",1,16));
					newPullOutRewLabel.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(KeyStroke.getKeyStroke(KeyEvent.VK_ESCAPE,0), "newPullOutRewLabel");
					newPullOutRewLabel.getActionMap().put("newPullOutRewLabel",getNewPullOutReqLabelAbstractAction() );
				}
				{
					jLabel2 = new JLabel();
...
//								ResultSet rs = db.executeQuery("SELECT * FROM clerk_lu WHERE AES_DECRYPT(password,'babyland') = '"+jPasswordField1.getText()+"' AND clerk_code = '"+jTextField1.getText()+"'");
	...
	private AbstractAction getNewPullOutReqLabelAbstractAction() {
		AbstractAction newPullOutReqLabelAction = new AbstractAction("New Pull Out Request", null) {
			
			public void actionPerformed(ActionEvent evt) {
				NewPullOutReq.this.dispose();
			}
		};
		return newPullOutReqLabelAction;
	}
