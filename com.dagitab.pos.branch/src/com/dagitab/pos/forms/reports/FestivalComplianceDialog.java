...
import com.dagitab.pos.bus.ComplianceMode;
import com.dagitab.pos.bus.FestivalComplianceService;
import com.dagitab.pos.main.Main;
import com.dagitab.pos.util.ComplianceFileReader;
...
public class FestivalComplianceDialog extends javax.swing.JDialog {
	...
	*/
	...
	public FestivalComplianceDialog(JFrame frame) {
		super(frame);
		initGUI();
		init();
	}
		...
		try {
			{
				...
				this.setTitle("Festival Compliance");
				...
				this.setModal(true);
				{
					...
					titleLabel.setText("Festival Compliance Form");
...
//									 int selected = lsm.getMaxSelectionIndex();
		...
		if(closeButtonAction == null) {
			closeButtonAction = new AbstractAction("Close", null) {
				public void actionPerformed(ActionEvent evt) {
					FestivalComplianceDialog.this.dispose();
				}
			};
		}
	...
	private void init(){
		...
		ResultSet rs = Main.getDBManager().executeQuery("SELECT REPORT_DATE, FILENAME FROM festival_compliance WHERE mode = \""+complianceMode.getComplianceSuffix()+"\" ORDER BY REPORT_DATE DESC LIMIT 30");
		...
		List<Date> listDates = FestivalComplianceService.getInstance().getUnsentComplianceReports(7, complianceMode);
	...
	private void filterSentItemTableByDate(int year, int month, int day, ComplianceMode complianceMode){
		String query = "SELECT REPORT_DATE, FILENAME FROM festival_compliance WHERE mode = \""+complianceMode.getComplianceSuffix()+"\"";
		...
		if(resendDateAction == null) {
			resendDateAction = new AbstractAction("Resend", null) {
				public void actionPerformed(ActionEvent evt) {
					...
					logger.info("generating festival compliace report for date: "+calendar.getTime());
		...
		if(viewLastDaysAction == null) {
			viewLastDaysAction = new AbstractAction("", null) {
				public void actionPerformed(ActionEvent evt) {
					...
					FestivalComplianceService festivalComplianceService = FestivalComplianceService.getInstance();
					List<Date> listDates = festivalComplianceService.getUnsentComplianceReports(days, complianceMode);
					String[][] data = new String[listDates.size()][1];
					for(int i = 0; i<listDates.size(); i++){
