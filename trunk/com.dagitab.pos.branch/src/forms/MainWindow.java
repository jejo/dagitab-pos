...
//				JOptionPane.showMessageDialog(null,"Please Restart Application","Restart",JOptionPane.INFORMATION_MESSAGE);
//				frame.dispose();
//		}
//		else{
		...
		try {
			{
				...
				{
					jLabel1 = new JLabel();
					getContentPane().add(jLabel1, new AnchorConstraint(13, 984, 63, 849, AnchorConstraint.ANCHOR_REL, AnchorConstraint.ANCHOR_REL, AnchorConstraint.ANCHOR_REL, AnchorConstraint.ANCHOR_REL));
				...
				}
				{
					...
					getContentPane().add(jPanel3, new AnchorConstraint(970, 1008, 1008, 0, AnchorConstraint.ANCHOR_REL, AnchorConstraint.ANCHOR_REL, AnchorConstraint.ANCHOR_REL, AnchorConstraint.ANCHOR_REL));
					...
					{
						jLabel3 = new JLabel();
						jPanel3.add(jLabel3, new AnchorConstraint(23, 95, 690, 8, AnchorConstraint.ANCHOR_REL, AnchorConstraint.ANCHOR_REL, AnchorConstraint.ANCHOR_REL, AnchorConstraint.ANCHOR_REL));
				...
				}
				{
					...
					getContentPane().add(jTabbedPane1, new AnchorConstraint(25, 992, 963, 8, AnchorConstraint.ANCHOR_REL, AnchorConstraint.ANCHOR_REL, AnchorConstraint.ANCHOR_REL, AnchorConstraint.ANCHOR_REL));
					jTabbedPane1.setPreferredSize(new java.awt.Dimension(868, 525));
...
//						jTabbedPane1.addTab("Pending Transactions",icon,pendingPanel,null);
			...
			jTable1.getSelectionModel().addListSelectionListener(new SelectionListener(jTable1){
				  public void valueChanged(ListSelectionEvent e) {
					  if (e.getSource() == jTable1.getSelectionModel()
			                ...
			                String prodcode = jTable1.getValueAt(jTable1.getSelectedRow(),0).toString();
			              
			                ResultSet rs = Main.getDBManager().executeQuery("SELECT * FROM products_lu " +
			        												  "WHERE PACKAGE = '1' " +
	...
	public void updateAmounts(){
		...
		for(int i = 0; i< productItems.size(); i++){
			int quantity =  Integer.parseInt(productItems.get(i).get(2).toString());
			double price = Double.parseDouble(productItems.get(i).get(4).toString());
		...
		for(int i =0; i<paymentItems.size(); i++){
			paymentAmount += Double.parseDouble(paymentItems.get(i).get(2).toString());
		}
	...
	private void processTransaction(String stat) throws SQLException{
		...
		if(n == 0){
			...
			//update invoice
			ResultSet res = Main.getDBManager().executeQuery("SELECT * FROM invoice LIMIT 1");
			res.moveToInsertRow();
			...
			String dataToLog = DataUtil.rowToData("ADD","invoice",keyFields,keys);
			cachewriter.writeToFile(dataToLog);
			
			//update invoice_item
			for(int i = 0; i< productItems.size(); i++){
				res = Main.getDBManager().executeQuery("SELECT * FROM invoice_item LIMIT 1");
				res.moveToInsertRow();
				res.updateObject("OR_NO", jTextField2.getText());
				res.updateObject("PROD_CODE", productItems.get(i).get(0).toString());
				res.updateObject("DISC_CODE", productItems.get(i).get(6).toString());
				res.updateObject("QUANTITY", productItems.get(i).get(2).toString());
				res.updateObject("STORE_CODE", Main.getStoreCode());
				...
				res.updateObject("SELL_PRICE", productItems.get(i).get(4).toString());
				
				ResultSet temprs = Main.getDBManager().executeQuery("SELECT COST FROM products_lu " +
												"WHERE PROD_CODE = \""+ productItems.get(i).get(0).toString()+"\"");
				if(temprs.next()){
				...
				String dataToLog2 = DataUtil.rowToData("ADD","invoice_item",
												...
												new String[]{jTextField2.getText(),productItems.get(i).get(0).toString(),Main.getStoreCode()});
			...
			for(int i = 0; i< paymentItems.size(); i++){
				...
				if(jTable2.getValueAt(i, 1).toString().equals("Cash")){
					...
					if(Double.parseDouble(jTextField8.getText()) > 0){
						amount = paymentCash - Double.parseDouble(jTextField8.getText());
					}
					else{
						amount = paymentCash;
					}
				...
				String query = "INSERT INTO payment_item " +
						...
						"("+jTextField2.getText()+","+paymentItems.get(i).get(0).toString()+","+ 
				...
				String dataToLog2 = DataUtil.rowToData("ADD","payment_item",
						...
						new String[]{jTextField2.getText(),paymentItems.get(i).get(0).toString(),
			...
			if(!stat.equals("partial")){
				 /** Make Receipt **/
					...
					rs = Main.getDBManager().executeQuery("SELECT * FROM clerk_lu WHERE CLERK_CODE = \""+jTextField6.getText()+ "\"");
					if(rs.next()){
				 ...
				 /*MAKE RECEIPT*/
					...
					ValidateReceipt receiptdialog = new ValidateReceipt(MainWindow.this,headerData, itemData, paymentData, vatAmount, changeAmount, Main.getDBManager(),"par");
					receiptdialog.setLocationRelativeTo(null);
					receiptdialog.setVisible(true);
	...
	public void setPartialList(){
		ResultSet rs = Main.getDBManager().executeQuery("SELECT OR_NO, TRANS_DT FROM invoice WHERE PARTIAL = 1");
		try {
			...
			TableModel jTable1Model = new DefaultTableModel(
			data,
			new String[] { "OR Number","Transaction Date & Time"  });
			jTable4 = new JTable(){
				public boolean isCellEditable(int row, int column)
				{
					return false;
	...
	private void setDeferredList(){
		ResultSet rs = Main.getDBManager().executeQuery("SELECT invoice_item.OR_NO, INVOICE_NO, invoice_item.PROD_CODE, NAME, QUANTITY, invoice.TRANS_DT  " +
		...
		try {
			...
			TableModel jTable1Model = new DefaultTableModel(
			data,
			new String[] { "OR Number", "Invoice Number","Product Code","Product Name","Quantity","Transaction Date & Time"  });
			jTable3 = new JTable(){
				public boolean isCellEditable(int row, int column)
				{
					return false;
	...
	public void setReturnedTable(Vector<String> args){
		...
		TableModel jTable1Model = new DefaultTableModel(
				data,
				new String[] { "Product Code", "Product Name","Quantity","Selling Price","Current Price","Deferred","Disc Code","Reason Code"  });
			jTable6 = new JTable(){
				public boolean isCellEditable(int row, int column)
				{
					return false;
...
//		update invoice
		Main.getDBManager().executeUpdate("UPDATE invoice SET `RETURN` = 1 WHERE OR_NO = "+jTextField20.getText()+" && STORE_CODE = '"+Main.getStoreCode()+"'");
		System.out.println("UPDATE invoice SET `RETURN` = 1 WHERE OR_NO = "+jTextField20.getText()+" && STORE_CODE = '"+Main.getStoreCode()+"'");
		ResultSet res = Main.getDBManager().executeQuery("SELECT * FROM invoice LIMIT 1");
		res.moveToInsertRow();
		...
		for(int i = 0; i< replacedItems.size(); i++){
			...
			res.updateObject("PROD_CODE", replacedItems.get(i).get(0).toString());
			res.updateObject("DISC_CODE", replacedItems.get(i).get(6).toString());
			res.updateObject("QUANTITY", replacedItems.get(i).get(2).toString());
			res.updateObject("STORE_CODE", Main.getStoreCode());
			...
			res.updateObject("SELL_PRICE", replacedItems.get(i).get(4).toString());
			
			ResultSet temprs = Main.getDBManager().executeQuery("SELECT COST FROM products_lu " +
											"WHERE PROD_CODE = '"+ replacedItems.get(i).get(0).toString()+"'");
			if(temprs.next()){
				res.updateObject("COST", temprs.getString("COST"));
			}
			res.insertRow();
			...
			String dataToLog2 = DataUtil.rowToData("ADD","invoice_item",
					new String[]{"OR_NO","PROD_CODE","STORE_CODE"}, 
					new String[]{jTextField11.getText(),replacedItems.get(i).get(0).toString(),Main.getStoreCode()});
			cachewriter.writeToFile(dataToLog2);
		}
		...
		for(int i = 0; i< replacedPaymentItems.size(); i++){
			...
			if(jTable7.getValueAt(i, 1).toString().equals("Cash")){
				paymentCash = Double.parseDouble(jTable7.getValueAt(i, 2).toString());
				if(Double.parseDouble(jTextField19.getText()) > 0){
					amount = paymentCash - Double.parseDouble(jTextField19.getText());
				}
				else{
					amount = paymentCash;
				}
				...
				amount = Double.parseDouble(jTable7.getValueAt(i, 2).toString());
			}
			String query = "INSERT INTO payment_item " +
					"(OR_NO, PT_CODE, AMT, CARD_TYPE, CARD_NO, GC_NO, CHECK_NO,STORE_CODE) VALUES " +
					"("+jTextField11.getText()+","+replacedPaymentItems.get(i).get(0).toString()+","+ 
					amount+",'"+replacedPaymentItems.get(i).get(3).toString()+"','"+
					replacedPaymentItems.get(i).get(4).toString()+"','"+replacedPaymentItems.get(i).get(6).toString()+"','"+
					replacedPaymentItems.get(i).get(5).toString()+"','"+Main.getStoreCode()+"')";
			...
			String dataToLog2 = DataUtil.rowToData("ADD","payment_item",
					new String[]{"OR_NO","PT_CODE","CARD_NO","GC_NO","CHECK_NO","STORE_CODE"}, 
					new String[]{jTextField11.getText(),replacedPaymentItems.get(i).get(0).toString(),
...
//	update amounts for returned items (subtotal, changes... etc)
	public void updateReturnedAmounts(){
		...
		for(int i = 0; i<returnedItems.size(); i++){
			int quantity =  Integer.parseInt(returnedItems.get(i).get(2).toString());
			double price = Double.parseDouble(returnedItems.get(i).get(3).toString());
		...
		for(int i = 0; i< replacedItems.size(); i++){
			int quantity =  Integer.parseInt(replacedItems.get(i).get(2).toString());
			double price = Double.parseDouble(replacedItems.get(i).get(4).toString());
			amount += (quantity*price);
		}
...
//			jLabel32.setText("0.00");
		...
		for(int i =0; i<replacedPaymentItems.size(); i++){
			paymentAmount += Double.parseDouble(replacedPaymentItems.get(i).get(2).toString());
		}
		...
		if(productItems.size()>0){
			...
			for(int i = 0; i< rowData.size(); i++){
				...
				for(int j = 0; j< productItems.size(); j++){
					...
					if(rowData.get(i).get(0).toString().equals(productItems.get(j).get(0).toString())){
					
						hasProduct = true;
						
						int fromnewquantity = Integer.parseInt(rowData.get(i).get(2).toString());
						int fromoldquantity = Integer.parseInt(productItems.get(j).get(2).toString());
						int newqty = fromoldquantity + fromnewquantity;
						ResultSet rs = Main.getDBManager().executeQuery("SELECT * FROM inventory_lu WHERE PROD_CODE = '"+rowData.get(i).get(0).toString()+"' AND STORE_CODE = '"+ Main.getStoreCode()+"'");
						try {
							if(rs.next()){
								...
								if(availableQuantity >= newqty){
									...
									JOptionPane.showMessageDialog(null, 
											"The item: "+rowData.get(i).get(0).toString()+", based on the addition of its quantity, exceed its limit from the inventory. \n You may only get "+availableQuantity, 
											"Warning",JOptionPane.WARNING_MESSAGE);
									productItems.get(j).set(2,availableQuantity+"");
								}
							}
						} catch (SQLException e1) {
							// TODO Auto-generated catch block
