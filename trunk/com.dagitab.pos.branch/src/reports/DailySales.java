...
// WHERE prod_code=\""+rs.getString("prod_code")+"\"");
			...
			ResultSet rs = db.executeQuery("SELECT i.or_no,i.store_code,i.cust_no,i.encoder_code FROM invoice i WHERE DATE(i.trans_dt)>=\'"+startDate + "\' && DATE(i.trans_dt) <= \'"+endDate +"\' "+branchClause);
			//System.out.println("SELECT i.or_no,i.store_code,i.cust_no,i.encoder_code FROM invoice i WHERE DATE(i.trans_dt)=\'"+startDate + "\'"+branchClause);
...
//				}
				...
				ResultSet rs2 = db.executeQuery("SELECT CONCAT(first_name,' ',last_name) FROM clerk_lu WHERE clerk_code="+rs.getInt(4));
				cell = createStringCell(wb,row,(short)1,false,true);
				if(rs2.next()) {	
					cell.setCellValue(rs2.getString(1));					
				}
				...
				cell = createAmountCell(wb,row,(short)2,false,true);
				if(rs2.next()) {
			...
			cell = createAmountCell(wb,row,(short)2,true,true);
